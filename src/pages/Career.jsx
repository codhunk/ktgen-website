...
const CareerPage = () => {
    ...
    const shortText = `Manmed Health Pvt. Ltd. is a trusted name in the pharmaceutical industry, dedicated to the branding, marketing, and nationwide distribution of high-quality medicinal products. With a strong commitment to innovation, compliance, and customer well-being, we specialize in creating impactful brand identities for a wide range of healthcare products—ensuring that essential medicines reach patients with both clarity and credibility.`;
    const moreText = `At Manmed Health, we bridge the gap between pharmaceutical manufacturers and healthcare providers through effective brand positioning, packaging design, and a robust supply chain network. Our goal is to not only represent the science behind the medicine but to enhance its accessibility and trustworthiness in the market. Driven by ethical business practices and a passion for healthcare, our team works relentlessly to meet evolving industry standards and build lasting partnerships with doctors, retailers, and institutions across India. As we continue to expand, Manmed Health Pvt. Ltd. remains committed to delivering wellness, one brand at a time.`;
    const experts = [
        {
